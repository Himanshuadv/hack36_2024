    ...
    <div className="flex flex-col w-full">
      ...
      <div className="flex flex-row justify-between items-center">
        <div className="flex flex-row w-2/6 justify-evenly">
          ...
          <PricetagsSharp height="28px" width="28px" color="#00ADB5" className="hover:-translate-y-1 transition-all duration-300 ease-in-out cursor-pointer" onClick={() => setIsTag(!isTag)} />
          
        </div>
        <div>
  {tag.length > 0 ? (
    <div className="overflow-scroll w-2/3" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
      {tag.map((tagItem, index) => (
        <span key={index} className="bg-white shadow-sm rounded-md shadow-gray-500 px-1 text-sm m-1">
          {tagItem}
        </span>
      ))}
    </div>
  ) : (
    <span className="bg-white shadow-sm rounded-md shadow-gray-500 px-1 text-sm">select tag</span>
  )}
</div>
      ...
      {isTag && (
        <div className="flex w-full mt-2 items-center z-50 p-2">
          {options.map((opt) => (
            <div key={opt.id} className="p-1 text-sm text-center rounded-md bg-gray-400 m-2 cursor-pointer" onClick={() => handleTag(opt.name)}>
              {opt.name}
            </div>
          ))}
        </div>
